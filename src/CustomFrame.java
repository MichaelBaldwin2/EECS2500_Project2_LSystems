...
class CustomFrame extends JFrame implements ActionListener {
	 ...
	 * Examples list.
	 */
	private String[] examples = {"Simple Tree", "Koch Curve I", "Koch Curve II", "Hilbert Curve", "Sierpinski Triangle", "Cube", "Crystal", "Hexagonal Gosper", "Rings", "Dragon Curve", "Seaweed", "Fractal Plant", "Penrose Tiling"};
	...
	public final void actionPerformed(final ActionEvent event) {
		if (event.getActionCommand() == "comboBoxChanged") {
			@SuppressWarnings("unchecked") JComboBox<String> examplesBox = (JComboBox<String>) event.getSource();
			...
			try {
				...
				int startPosition = 1;
				if (!centerButton.isSelected()) startPosition = cornerButton.isSelected() ? 2 : 3;

				Project2.calculateLSystem(start, rules, iterations, angle, lineLength, startPosition, verticalCheckBox.isSelected());

			} catch (NumberFormatException exception) {
				JOptionPane.showMessageDialog(canvasPanel, "Please review input parameters.", "Error", JOptionPane.ERROR_MESSAGE);
			}
		}
	}
}
